...
- [ ] Điền `.env` production: `DATABASE_URL` (trỏ vào service `db` cùng compose, VD `postgresql://cozypomo:<DB_PASSWORD>@db:5432/cozypomo?schema=public`), `JWT_SECRET`, `JWT_REFRESH_SECRET`, `DB_PASSWORD`, `ADMIN_EMAIL`/`ADMIN_PASSWORD`/`ADMIN_COOKIE_SECRET` (đăng nhập `/admin`)
- [ ] (Tuỳ chọn) `npm run prisma:seed` để nạp dữ liệu demo (175 loài + 10 tài khoản tester) — xem `backend/README.md`
...
| NestJS backend (`backend/`) | ✅ **Hoàn chỉnh** — toàn bộ API (Auth/Species/Eggs/Sessions/Collection/Currency/Shop/Stats/Settings/Sync) + trang quản trị AdminJS (`/admin`) + seed 175 loài/10 tester, đã test thật (curl toàn bộ endpoint, build Docker image thật, chạy trong container thật với Postgres thật) |
